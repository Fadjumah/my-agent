  ...
  try {
    ...
    CLOUD_KEYS.forEach(function(k) {
      ...
      if (IDENTITY_KEYS.indexOf(k) >= 0) {
        // Cloud wins for identity/config keys — but only if cloud has actual data
        var cloudVal = data[k];
        var isEmpty = cloudVal === null || cloudVal === undefined ||
          (Array.isArray(cloudVal) && cloudVal.length === 0) ||
          (typeof cloudVal === 'string' && cloudVal.trim() === '');
        if (!isEmpty) m[k] = cloudVal;
      } else if (k === 'sessions') {
...
// ── Delete all chats — wipes local + cloud sessions ──────────────────────
async function deleteAllChats() {
  ...
  if (window.getSessionToken()) {
    fetch('/api/sync', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Agent-Token': window.getSessionToken() },
      body: JSON.stringify({ action: 'set', key: 'sessions', value: [] })
    }).catch(function(){});
    fetch('/api/sync', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Agent-Token': window.getSessionToken() },
      body: JSON.stringify({ action: 'set', key: 'currentSession', value: null })
    }).catch(function(){});
  }
}
...
Object.assign(window, {
  ...
  syncKeyToCloud, syncFromCloud, pushCrossMemory, getCrossMemorySummary, deleteAllChats, CLOUD_KEYS,
