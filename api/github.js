  ...
  try {
    if (action === 'listRepos') {
      ...
      return res.status(200).json({
        repos: data.map(r => ({ name: r.full_name, description: r.description || '', private: r.private, language: r.language || '', updatedAt: r.updated_at, url: r.html_url, defaultBranch: r.default_branch })),
        total: data.length,
      });
    }
    ...
    if (action === 'getRepo') {
      ...
      if (status !== 200) return res.status(status).json({ error: errMsg || 'GitHub HTTP ' + status });
      return res.status(200).json({ name: data.full_name, description: data.description, private: data.private, language: data.language, stars: data.stargazers_count, defaultBranch: data.default_branch, url: data.html_url, createdAt: data.created_at, updatedAt: data.updated_at });
    }
    ...
    if (action === 'listFiles') {
      ...
      if (status !== 200) return res.status(status).json({ error: errMsg || 'GitHub HTTP ' + status });
      return res.status(200).json({ files: Array.isArray(data) ? data.map(f => ({ name: f.name, path: f.path, type: f.type, size: f.size })) : [] });
    }
    ...
    if (action === 'listCommits') {
      ...
      return res.status(200).json({ commits: data.map(c => ({ sha: c.sha.slice(0, 7), message: c.commit.message, author: c.commit.author.name, date: c.commit.author.date })) });
    }
    ...
    if (action === 'pushFile') {
      ...
      const [status, data, errMsg] = await gh(`https://api.github.com/repos/${repo}/contents/${path}`, { method: 'PUT', body: JSON.stringify(putBody) });
